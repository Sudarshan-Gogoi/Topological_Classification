FindCorrMat <- function(h5_filename = NULL, 
  ...
  barcodes_filepath <- if (!is.null(barcodes_filename)) file.path(base_path, barcodes_filename) else NULL
  features_filepath <- if (!is.null(features_filename)) file.path(base_path, features_filename) else NULL
  matrix_filepath <- if (!is.null(matrix_filename)) file.path(base_path, matrix_filename) else NULL
  ...
  if (!is.null(h5_filename)) {
    ...
    # Handle multi-modal h5
    if (is.list(Cancer.sparse.m)) {
      if ("Gene Expression" %in% names(Cancer.sparse.m)) {
        cts <- Cancer.sparse.m$`Gene Expression`
      } else {
        cts <- Cancer.sparse.m[[1]]
      }
    } else {
      cts <- Cancer.sparse.m
    }
    ...
    if (is.null(barcodes_path) || is.null(features_path) || is.null(matrix_path)) {
      stop("Could not find all required files (barcodes, features, matrix).")
    }
    ...
    cts <- ReadMtx(
      ...
      cells = barcodes_path
             ...
             !is.null(features_filename) && 
             !is.null(matrix_filename)) {
    
    message("Loading matrix, features and barcodes files...")
    
    cts <- ReadMtx(
      mtx = matrix_filepath,
      features = features_filepath,
      cells = barcodes_filepath
    )
    ...
    stop("Please provide one of the following:
         1) h5_filename
         2) geo_tar_filename
         3) barcodes_filename + features_filename + matrix_filename")
  }
  ...
  Cancer.seurat.obj[["percent.mt"]] <- PercentageFeatureSet(
    Cancer.seurat.obj,
    pattern = "^MT-"
  )
  ...
  Cancer.seurat.obj <- NormalizeData(
    Cancer.seurat.obj,
  ...
  Cancer.seurat.obj <- FindVariableFeatures(
    Cancer.seurat.obj,
  ...
  Cancer.seurat.obj <- RunPCA(
    Cancer.seurat.obj,
    features = VariableFeatures(object = Cancer.seurat.obj)
  )
  ...
  for (pc in 1:num_pcs) {
    ...
    ranked_genes <- rownames(gene_loadings)[
      order(abs(gene_loadings[, pc]), decreasing = TRUE)
    ]
  ...
  seurat_df <- as.data.frame(
    GetAssayData(Cancer.seurat.obj, layer = "data")
  )
  
  filtered_df <- seurat_df[
    rownames(seurat_df) %in% unique_genes,
  ]
  ...
  fwrite(
    x = filtered_correlation,
    row.names = TRUE,
    file = correlation_matrix_path
  )
...
# Using 10x H5 file (Example):
FindCorrMat(
  h5_filename = "GSE185224_Donor3_filtered_feature_bc_matrix.h5",
  base_path = "D:/Topologicalcancer-signatures"
)

# Using GEO tar file (Example):
FindCorrMat(
  geo_tar_filename = "GSE235168_RAW.tar",
  base_path = "D:/Topologicalcancer-signatures"
)

# Using separate files of Features-Barcodes_Matrix (Example):
FindCorrMat(
